...
export default function NewPostPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name === "title" || name === "author" || name === "content" || name === "excerpt") {
      // Capitalize the first letter as the user types, and trim leading spaces
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError(null);
    // Trim all form fields before sending
  ...
  return (
    <form className="mb-32" onSubmit={handleSubmit}>
      ...
      {/* Imput for image or video cover */}
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col items-center w-full px-10 lg:px-32">
          <button
            type="submit"
            className="bg-black hover:bg-white hover:text-black border border-black text-white font-bold py-2 duration-200 transition-colors mb-6 lg:mb-0 w-full "
            disabled={loading}
          >
            {loading ? "Saving..." : "Save"}
          </button>
